    ...
    <div class="container">
        <div class="row">
            <div class="col-md-8 offset-md-2">
                ...
                <h4>
                    <span class="small-text text-muted float-left">{{product.price}} {{product.currency}}</span>
                    <span class="small-text float-right">Available Quantity: {{product.units}}</span>
                </h4>
                <br>
                <hr>
                <router-link :to="{ path: '/checkout?pid='+product.id }" class="col-md-4 btn btn-sm btn-dark float-right">Buy Now</router-link>
            </div>
        </div>
    </div>
</template>
